...
and string_of_cexpr_with (depth : int) (print_a : 'a -> string) (c : 'a cexpr) : string =
  ...
  | CPrim2(op, left, right, a) ->
     sprintf "(%s %s %s)%s" (string_of_aexpr_with depth print_a left) (string_of_op2 op) (string_of_aexpr_with depth print_a right) (print_a a)
  | CIf(cond, thn, els, a) ->
     sprintf "(if %s: %s else: %s)%s"
