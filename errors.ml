...
exception ExpectedTupleTyp of sourcespan typ * sourcespan expr * sourcespan

(* The type that was found that was expected to be a tuple type and the expr that had that type*)
...
let print_errors (exns : exn list) : string list =
  List.map
    (fun e ->
      ...
      | UnboundId (x, loc) -> sprintf "The identifier %s, used at <%s>, is not in scope" x (string_of_sourcespan loc)
      | UnboundTyId (x, loc) -> sprintf "The type name %s, used at <%s>, is not in scope" x (string_of_sourcespan loc)
      | UnboundFun (x, loc) ->
      ...
      | DuplicateFun (x, loc, existing) ->
        sprintf
          ...
          (string_of_sourcespan existing)
      | Overflow (num, loc) ->
        sprintf "The number literal %Ld, used at <%s>, is not supported in this language" num (string_of_sourcespan loc)
      | VarNoType (name, loc) -> sprintf "The variable %s at <%s> has no type defined" name (string_of_sourcespan loc)
      | NoType (name, loc) -> sprintf "The function %s at <%s> has no type defined" name (string_of_sourcespan loc)
      | BodyNoType loc -> sprintf "The program body at <%s> has no type defined" (string_of_sourcespan loc)
      | ExprNoType (e, loc) ->
        sprintf "The expr '%s' at <%s> has no type defined" (string_of_expr e) (string_of_sourcespan loc)
      | BadTypeApp (name, loc, expected, actual) ->
        sprintf
          "The function %s, called at %s, expected %d type arguments but only %d types provided"
          name
          (string_of_sourcespan loc)
          expected
          actual
      | BadTypeAppTyDecl (name, loc, expected, actual) ->
        sprintf
          "The tydecl %s, instantiated at %s, expected %d type arguments but only %d types provided"
          name
          (string_of_sourcespan loc)
          expected
          actual
      ...
      | ShouldBeFunction (name, loc, wanted) ->
        sprintf
          "The function %s, at %s, should have function type %s"
          name
          (string_of_sourcespan loc)
          (string_of_scheme wanted)
      ...
      | TypeMismatch (loc, expected, actual, []) ->
        sprintf
          "Type error at %s: expected %s but got %s"
          (string_of_sourcespan loc)
          (string_of_typ expected)
          (string_of_typ actual)
      | IncorrectTupleLength (expected, actual, loc) ->
        sprintf
          "Type error at %s: tuple is of length %d, but was accessed as if it was of length %d"
          (string_of_sourcespan loc)
          actual
          expected
      | ExpectedTupleTyp (t, e, l) ->
        sprintf
          "Type error at %s: Expected to find a tuple type for expr %s but instead found type %s"
          (string_of_sourcespan l)
          (string_of_expr e)
          (string_of_typ t)
      | TypeMismatch (loc, expected, actual, reasons) ->
        let print_reason r =
          match r with
          | InferExp e ->
            sprintf "\ttrying to infer type for %s at %s" (string_of_expr e) (string_of_sourcespan (get_tag_E e))
          | Message s -> "\t" ^ s
          | Unify (s, t1, t2) ->
            sprintf "\ttrying to unify %s and %s (because %s)" (string_of_typ t1) (string_of_typ t2) s
          | Instantiate (name, scheme) -> sprintf "\ttrying to instantiate %s at %s" name (string_of_scheme scheme)
        in
        sprintf
          "Type error at %s: expected %s but got %s, because\n%s"
          (string_of_sourcespan loc)
          (string_of_typ expected)
          (string_of_typ actual)
          (ExtString.String.join "\n" (List.map print_reason reasons))
      | IndexOutOfBounds (idx, len, loc) ->
        sprintf "Tuple indexing at %s is out of bounds (idx=%d, len=%d)" (string_of_sourcespan loc) idx len
