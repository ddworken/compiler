...
let () =
  ...
  | Error (errs, trace) ->
    if !show_trace then eprintf "%s%s" (ExtString.String.join sep (print_trace trace)) sep else ();
    eprintf "Errors:\n";
    eprintf "%s\n" (ExtString.String.join "\n" (print_errors errs))
  | Ok (program, trace) ->
    if !show_trace then printf "%s\n" (ExtString.String.join sep (print_trace trace)) else printf "%s\n" program
;;
