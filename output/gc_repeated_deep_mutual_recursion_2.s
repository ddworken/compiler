...
lambda_18:
  ...
  sub RAX, 0x5 ; And detag it. Our compiler guarantees that it will be a lambda so no need to tag check
  mov R11, [RAX+24]
  mov [RBP-8], R11 ; Holds the closed over variable: f2_80
  ;; } Copy the free variables off the heap onto the stack
  mov RAX, [RBP+16]
  mov R11, RAX
  mov RAX, 0
  ...
  mov RAX, R11
  cmp RAX, R12
  jne near _cmp_fail_38
  mov RAX, 0xffffffffffffffff
  jmp near _cmp_done_38
_cmp_fail_38:
...
memcheck_ok_29:
  ...
  mov [RBP-24], RAX ; [RBP-24] holds the variable binop_73
  mov RAX, [RBP-8]
...
lambda_end_18:
  ...
  mov R15, RAX ; assume gc success if returning here, so RAX holds the new heap_reg value
memcheck_ok_18:
  ;; Store lambda-18 on the heap {
  mov QWORD [R15+0], 2 ; Store the arity
  mov QWORD [R15+8], lambda_18 ; Store the function pointer
  mov QWORD [R15+16], 2 ; Store the number of closed variables
  ;; Store all the closed over variables {
  mov QWORD [R15+24], 24690 ; reserved space, will be filled in with: f2_80
  ;; } Store all the closed over variables
  mov RAX, R15 ; Move the address of the tuple into RAX
  add RAX, 0x5 ; Tag it as a closure
  add R15, 32 ; Bump the heap register
...
lambda_41:
  ...
  mov RAX, R11
  cmp RAX, R12
  jne near _cmp_fail_61
  mov RAX, 0xffffffffffffffff
  jmp near _cmp_done_61
_cmp_fail_61:
  mov RAX, 0x7fffffffffffffff
_cmp_done_61:
  mov [RBP-16], RAX ; [RBP-16] holds the variable binop_84
  mov RAX, [RBP-16]
  ;; Assert that RAX has the tag 7 (aka: 'bool')
  mov R11, RAX
  shl R11, 61
...
memcheck_ok_52:
  ...
  sub RAX, R12
  jo near _global_overflow_abort
  mov [RBP-24], RAX ; [RBP-24] holds the variable binop_100
  mov RAX, [RBP-8]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
...
memcheck_ok_41:
  ...
  add R15, 32 ; Bump the heap register
  ;; } store lambda-41
  mov [RBP-56], RAX ; Store letrec-ed function f2_80
  mov RAX, [RBP-48]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
