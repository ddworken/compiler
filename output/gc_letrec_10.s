...
memcheck_ok_65:
  ;; Store lambda-65 on the heap {
  mov QWORD [R15+0], 2 ; Store the arity
  mov QWORD [R15+8], lambda_65 ; Store the function pointer
...
memcheck_ok_42:
  ...
  mov QWORD RAX, QWORD 220
  mov QWORD [R15+8], QWORD RAX
  mov QWORD RAX, QWORD 240
  mov QWORD [R15+16], QWORD RAX
  mov QWORD RAX, QWORD 260
...
memcheck_ok_37:
  ;; Creating a tuple: 
  mov QWORD [R15+0], 8 ; tuple size
  mov QWORD RAX, QWORD 280
  mov QWORD [R15+8], QWORD RAX
  mov QWORD RAX, QWORD 300
  mov QWORD [R15+16], QWORD RAX
  mov QWORD RAX, QWORD 320
  mov QWORD [R15+24], QWORD RAX
  mov QWORD RAX, QWORD 340
  mov QWORD [R15+32], QWORD RAX
  mov RAX, R15
  add RAX, 0x1
  add R15, 48
  ;; Check if we have space on the heap to store our tuple of size 64: 
...
memcheck_ok_30:
  ;; Creating a tuple: 
  mov QWORD [R15+0], 12 ; tuple size
  mov QWORD RAX, QWORD 360
  mov QWORD [R15+8], QWORD RAX
  mov QWORD RAX, QWORD 380
  mov QWORD [R15+16], QWORD RAX
  mov QWORD RAX, QWORD 400
  mov QWORD [R15+24], QWORD RAX
  mov QWORD RAX, QWORD 420
  mov QWORD [R15+32], QWORD RAX
  mov QWORD RAX, QWORD 440
  mov QWORD [R15+40], QWORD RAX
  mov QWORD RAX, QWORD 460
  mov QWORD [R15+48], QWORD RAX
  mov RAX, R15
  add RAX, 0x1
  add R15, 64
  ;; Define lambda-16 {
  jmp near lambda_end_16
lambda_16:
  ;; Make sure we have space left on the stack:
  mov R11, RBP
  sub R11, 16
  ...
  sub RAX, 0x5 ; And detag it. Our compiler guarantees that it will be a lambda so no need to tag check
  mov R11, [RAX+24]
  mov [RBP-8], R11 ; Holds the closed over variable: g_104
  ;; } Copy the free variables off the heap onto the stack
  mov RAX, [RBP+24]
  mov RAX, [RBP-8]
  mov RAX, [RBP+16]
...
lambda_end_16:
  ...
  mov R15, RAX ; assume gc success if returning here, so RAX holds the new heap_reg value
memcheck_ok_16:
  ;; Store lambda-16 on the heap {
  mov QWORD [R15+0], 2 ; Store the arity
  mov QWORD [R15+8], lambda_16 ; Store the function pointer
  mov QWORD [R15+16], 2 ; Store the number of closed variables
  ;; Store all the closed over variables {
  mov QWORD [R15+24], 24690 ; reserved space, will be filled in with: g_104
  ;; } Store all the closed over variables
  mov RAX, R15 ; Move the address of the tuple into RAX
  add RAX, 0x5 ; Tag it as a closure
  add R15, 32 ; Bump the heap register
  ;; } store lambda-16
  mov [RBP-48], RAX ; Store letrec-ed function f_87
  ;; Define lambda-22 {
...
lambda_22:
  ...
  sub RAX, 0x5 ; And detag it. Our compiler guarantees that it will be a lambda so no need to tag check
  mov R11, [RAX+24]
  mov [RBP-8], R11 ; Holds the closed over variable: f_87
  ;; } Copy the free variables off the heap onto the stack
  mov RAX, [RBP-8]
  mov RAX, [RBP+32]
  mov RAX, [RBP+16]
...
memcheck_ok_22:
  ...
  add R15, 32 ; Bump the heap register
  ;; } store lambda-22
  mov [RBP-56], RAX ; Store letrec-ed function g_104
  ;; Check if we have space on the heap to store our tuple of size 32: 
  mov RAX, [HEAP_END] ; Reserving 4 words
  sub RAX, 32
  cmp RAX, R15
...
memcheck_ok_13:
  ...
  add RAX, 0x1
  add R15, 32
  mov [RBP-64], RAX ; [RBP-64] holds the variable x_126
  mov RAX, [RBP-48]
  leave
  ret
  ;; Error labels: 
