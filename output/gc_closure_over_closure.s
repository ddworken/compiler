...
lambda_65:
  ...
  mov RAX, R11
  cmp RAX, R12
  jne near _cmp_fail_82
  mov RAX, 0xffffffffffffffff
  jmp near _cmp_done_82
_cmp_fail_82:
...
memcheck_ok_59:
  ...
  add RAX, 0x1
  add R15, 32
  mov [RBP-56], RAX ; [RBP-56] holds the variable tup_89
  ;; Check if we have space on the heap to store our tuple of size 32: 
  mov RAX, [HEAP_END] ; Reserving 4 words
  sub RAX, 32
  cmp RAX, R15
...
memcheck_ok_56:
  ...
  add RAX, 0x1
  add R15, 32
  mov [RBP-64], RAX ; [RBP-64] holds the variable a_85
...
memcheck_ok_51:
  ...
  add RAX, 0x1
  add R15, 48
  mov [RBP-72], RAX ; [RBP-72] holds the variable b_93
  ;; Define lambda-41 {
  jmp near lambda_end_41
lambda_41:
  ;; Make sure we have space left on the stack:
  mov R11, RBP
  sub R11, 32
  ...
  sub RAX, 0x5 ; And detag it. Our compiler guarantees that it will be a lambda so no need to tag check
  mov R11, [RAX+24]
  mov [RBP-8], R11 ; Holds the closed over variable: a_85
  mov R11, [RAX+32]
  mov [RBP-16], R11 ; Holds the closed over variable: print_4
  mov R11, [RAX+40]
  mov [RBP-24], R11 ; Holds the closed over variable: b_93
  ;; } Copy the free variables off the heap onto the stack
  mov RAX, [RBP-16]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
...
lambda_end_41:
  ...
  mov R15, RAX ; assume gc success if returning here, so RAX holds the new heap_reg value
memcheck_ok_41:
  ;; Store lambda-41 on the heap {
  mov QWORD [R15+0], 2 ; Store the arity
  mov QWORD [R15+8], lambda_41 ; Store the function pointer
  mov QWORD [R15+16], 6 ; Store the number of closed variables
  ;; Store all the closed over variables {
  mov R11, [RBP-64] ; Store a_85
  mov QWORD [R15+24], R11
  mov R11, [RBP-8] ; Store print_4
  mov QWORD [R15+32], R11
  mov R11, [RBP-72] ; Store b_93
  ...
  ;; } store lambda-41
  mov [RBP-80], RAX ; [RBP-80] holds the variable c_101
  ;; Define lambda-21 {
...
lambda_21:
  ...
  mov [RBP-8], R11 ; Holds the closed over variable: a_85
  mov R11, [RAX+32]
  mov [RBP-16], R11 ; Holds the closed over variable: b_93
  mov R11, [RAX+40]
  mov [RBP-24], R11 ; Holds the closed over variable: print_4
  mov R11, [RAX+48]
  mov [RBP-32], R11 ; Holds the closed over variable: c_101
  ;; } Copy the free variables off the heap onto the stack
  mov RAX, [RBP-24]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  ...
  add RAX, R12
  jo near _global_overflow_abort
  mov [RBP-40], RAX ; [RBP-40] holds the variable binop_147
  mov RAX, [RBP-32]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
...
memcheck_ok_21:
  ...
  ;; } store lambda-21
  mov [RBP-88], RAX ; [RBP-88] holds the variable d_122
  mov RAX, [RBP-48]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func(200)
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-96], RAX ; [RBP-96] holds the variable tmp_156
  mov RAX, [RBP-88]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
