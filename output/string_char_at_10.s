...
lambda_end_23:
  ...
  mov R15, RAX ; assume gc success if returning here, so RAX holds the new heap_reg value
memcheck_ok_23:
  ;; Store lambda-23 on the heap {
  mov QWORD [R15+0], 4 ; Store the arity
  mov QWORD [R15+8], lambda_23 ; Store the function pointer
  mov QWORD [R15+16], 0 ; Store the number of closed variables
  ;; Store all the closed over variables {
  ;; } Store all the closed over variables
  mov RAX, R15 ; Move the address of the tuple into RAX
...
lambda_end_19:
  ...
  mov R15, RAX ; assume gc success if returning here, so RAX holds the new heap_reg value
memcheck_ok_19:
  ;; Store lambda-19 on the heap {
  mov QWORD [R15+0], 2 ; Store the arity
  mov QWORD [R15+8], lambda_19 ; Store the function pointer
  mov QWORD [R15+16], 0 ; Store the number of closed variables
  ;; Store all the closed over variables {
  ;; } Store all the closed over variables
  mov RAX, R15 ; Move the address of the tuple into RAX
...
memcheck_ok_14:
  ...
  ;; Allocate space for the string 'true' on the heap:
...
memcheck_ok_13:
  ...
  mov BYTE [R15+3], 101
  mov BYTE [R15+4], 0
  ;; Then tag it and move it to RAX
  mov RAX, R15
  add RAX, 0x3
  add R15, 16
  mov [RBP-48], RAX ; [RBP-48] holds the variable string_52
  ;; Allocate space for the string '1' on the heap:
  mov RAX, [HEAP_END] ; Reserving 2 words
  sub RAX, 16
  cmp RAX, R15
...
memcheck_ok_12:
  ...
  add RAX, 0x3
  add R15, 16
  mov [RBP-56], RAX ; [RBP-56] holds the variable string_53
  mov RAX, [RBP-40]
