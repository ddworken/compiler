...
lambda_13:
  ...
  mov RAX, R11
  cmp RAX, R12
  jne near _cmp_fail_30
  mov RAX, 0xffffffffffffffff
  jmp near _cmp_done_30
_cmp_fail_30:
...
memcheck_ok_24:
  ...
  add RAX, 0x1
  add R15, 32
  mov [RBP-16], RAX ; [RBP-16] holds the variable x_63
  mov RAX, [RBP+16]
  mov R11, RAX
  ...
  mov [RBP-24], RAX ; [RBP-24] holds the variable binop_70
  mov RAX, [RBP+24]
...
memcheck_ok_13:
  ...
  add R15, 32 ; Bump the heap register
  ;; } store lambda-13
  mov [RBP-48], RAX ; Store letrec-ed function use_space_53
  mov RAX, [RBP-16]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func()
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-56], RAX ; [RBP-56] holds the variable app_77
  mov RAX, [RBP-48]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
