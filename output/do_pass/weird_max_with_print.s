...
lambda_11:
  ...
  mov RAX, [RBP-16]
  mov [RBP-24], RAX ; [RBP-24] holds the variable x_58
  mov RAX, [RBP-8]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-32], RAX ; [RBP-32] holds the variable app_66
  mov RAX, [RBP-8]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-40], RAX ; [RBP-40] holds the variable app_70
  mov RAX, [RBP-32]
  mov R11, RAX
  mov RAX, [RBP-40]
  mov R12, RAX
  ...
  mov RAX, R11
  cmp RAX, R12
  jl near _cmp_fail_24
  mov RAX, 0xffffffffffffffff
  jmp near _cmp_done_24
_cmp_fail_24:
  mov RAX, 0x7fffffffffffffff
_cmp_done_24:
  mov [RBP-48], RAX ; [RBP-48] holds the variable binop_65
  mov RAX, [RBP-48]
...
memcheck_ok_11:
  ...
  add R15, 32 ; Bump the heap register
  ;; } store lambda-11
  mov [RBP-48], RAX ; Store letrec-ed function max_53
  mov RAX, [RBP-48]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
