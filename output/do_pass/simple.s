...
memcheck_ok_38:
  ...
  mov QWORD RAX, QWORD 0xffffffffffffffff
  ...
  add RAX, 0x1
  add R15, 32
  mov [RBP-48], RAX ; [RBP-48] holds the variable tup_58
  ;; Check if we have space on the heap to store our tuple of size 32: 
  mov RAX, [HEAP_END] ; Reserving 4 words
  sub RAX, 32
  cmp RAX, R15
...
memcheck_ok_35:
  ...
  add RAX, 0x1
  add R15, 32
  mov [RBP-56], RAX ; [RBP-56] holds the variable tup_56
  mov RAX, [RBP-8]
...
memcheck_ok_23:
  ...
  add RAX, 0x1
  add R15, 32
  mov [RBP-64], RAX ; [RBP-64] holds the variable tup_83
  ;; Check if we have space on the heap to store our tuple of size 32: 
  mov RAX, [HEAP_END] ; Reserving 4 words
  sub RAX, 32
  cmp RAX, R15
...
memcheck_ok_20:
  ...
  add RAX, 0x1
  add R15, 32
  mov [RBP-72], RAX ; [RBP-72] holds the variable tup_81
  ;; Check if we have space on the heap to store our tuple of size 32: 
  mov RAX, [HEAP_END] ; Reserving 4 words
  sub RAX, 32
  cmp RAX, R15
...
memcheck_ok_17:
  ...
  mov QWORD RAX, QWORD 4
  mov QWORD [R15+8], QWORD RAX
  mov QWORD RAX, QWORD [RBP-72]
  mov QWORD [R15+16], QWORD RAX
  mov RAX, R15
  add RAX, 0x1
  add R15, 32
  mov [RBP-80], RAX ; [RBP-80] holds the variable tup_79
  ;; Check if we have space on the heap to store our tuple of size 32: 
  mov RAX, [HEAP_END] ; Reserving 4 words
  sub RAX, 32
  cmp RAX, R15
