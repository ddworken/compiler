...
memcheck_ok_107:
  ;; Store lambda-107 on the heap {
  mov QWORD [R15+0], 2 ; Store the arity
  mov QWORD [R15+8], lambda_107 ; Store the function pointer
...
lambda_end_104:
  ...
  mov R15, RAX ; assume gc success if returning here, so RAX holds the new heap_reg value
memcheck_ok_104:
  ;; Store lambda-104 on the heap {
  mov QWORD [R15+0], 0 ; Store the arity
  mov QWORD [R15+8], lambda_104 ; Store the function pointer
  mov QWORD [R15+16], 0 ; Store the number of closed variables
  ;; Store all the closed over variables {
  ;; } Store all the closed over variables
  mov RAX, R15 ; Move the address of the tuple into RAX
...
lambda_12:
  ...
  mov [RBP-16], RAX ; [RBP-16] holds the variable app_60
  mov RAX, [RBP-16]
  mov [RBP-24], RAX ; [RBP-24] holds the variable _1_58
  mov RAX, [RBP-8]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-32], RAX ; [RBP-32] holds the variable app_67
  mov RAX, [RBP-32]
  mov [RBP-40], RAX ; [RBP-40] holds the variable _2_65
  mov RAX, [RBP-8]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-48], RAX ; [RBP-48] holds the variable app_74
  mov RAX, [RBP-48]
  mov [RBP-56], RAX ; [RBP-56] holds the variable _3_72
  ;; and_lhs {
  ;; and_lhs {
  mov RAX, [RBP+16]
  mov R11, RAX
...
_cmp_done_68:
  ...
  mov RAX, 0xffffffffffffffff
and_done_67:
  mov [RBP-64], RAX ; [RBP-64] holds the variable binop_and_or_79
  mov RAX, [RBP-64]
  ;; Assert that RAX has the tag 7 (aka: 'bool')
  mov R11, RAX
  shl R11, 61
  ...
  mov [RBP-72], RAX ; [RBP-72] holds the variable binop_94
  mov RAX, [RBP-72]
  mov R11, RAX
  ;; Assert that R11 holds a number
  test QWORD R11, 0x1
...
_cmp_fail_56:
  mov RAX, 0x7fffffffffffffff
_cmp_done_56:
  mov [RBP-72], RAX ; [RBP-72] holds the variable binop_99
  mov RAX, [RBP-72]
  ;; Assert that RAX has the tag 7 (aka: 'bool')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func(-2)
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-80], RAX ; [RBP-80] holds the variable app_107
  mov RAX, [RBP-80]
  mov RAX, [RBP-8]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  ...
  call RAX ; func([RBP+24])
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-88], RAX ; [RBP-88] holds the variable app_112
  mov RAX, [RBP-8]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func([RBP+32])
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-96], RAX ; [RBP-96] holds the variable app_116
  mov RAX, [RBP-8]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func([RBP+16])
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-104], RAX ; [RBP-104] holds the variable app_120
  mov RAX, [RBP+40]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
...
if_false_22:
  ...
  sub RAX, R12
  jo near _global_overflow_abort
  mov [RBP-80], RAX ; [RBP-80] holds the variable binop_126
  mov RAX, [RBP+40]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  mov R11, [RBP+40] ; Pass the function itself as the final argument
  mov QWORD [RSP+24], R11
  call RAX ; func([RBP-80], [RBP+24], [RBP+32])
  add RSP, 32 ; Increment RSP after calling the closure
done_22:
done_20:
  leave
