...
lambda_10:
  ...
  mov RAX, R11
  cmp RAX, R12
  jg near _cmp_fail_34
  mov RAX, 0xffffffffffffffff
  jmp near _cmp_done_34
_cmp_fail_34:
...
if_false_12:
  ...
  mov [RBP-24], RAX ; [RBP-24] holds the variable app_62
  mov RAX, [RBP+16]
  mov R11, RAX
  ;; Assert that R11 holds a number
  test QWORD R11, 0x1
  ...
  sub RAX, R12
  jo near _global_overflow_abort
  mov [RBP-32], RAX ; [RBP-32] holds the variable binop_68
  mov RAX, [RBP+24]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
...
memcheck_ok_10:
  ...
  add R15, 32 ; Bump the heap register
  ;; } store lambda-10
  mov [RBP-48], RAX ; Store letrec-ed function fib_53
  mov RAX, [RBP-48]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
