...
lambda_14:
  ...
  mov RAX, [RAX+16]
  mov [RBP-8], RAX ; [RBP-8] holds the variable get_57
  mov RAX, [RBP+16]
  ;; Assert that RAX has the tag 1 (aka: 'tuple')
  mov R11, RAX
  shl R11, 61
  ...
  mov RAX, [RAX+8]
  mov [RBP-16], RAX ; [RBP-16] holds the variable get_59
...
memcheck_ok_14:
  ...
  add R15, 32 ; Bump the heap register
  ;; } store lambda-14
  mov [RBP-48], RAX ; Store letrec-ed function f_53
  ;; Check if we have space on the heap to store our tuple of size 32: 
  mov RAX, [HEAP_END] ; Reserving 4 words
  sub RAX, 32
  cmp RAX, R15
...
memcheck_ok_11:
  ...
  mov QWORD [R15+8], QWORD RAX
  mov QWORD RAX, QWORD 0x7fffffffffffffff
  mov QWORD [R15+16], QWORD RAX
  mov RAX, R15
  add RAX, 0x1
  add R15, 32
  mov [RBP-56], RAX ; [RBP-56] holds the variable x_71
  mov RAX, [RBP-48]
