...
lambda_15:
  ...
  sub RAX, 0x5 ; And detag it. Our compiler guarantees that it will be a lambda so no need to tag check
  mov R11, [RAX+24]
  mov [RBP-8], R11 ; Holds the closed over variable: lam1_71
  ;; } Copy the free variables off the heap onto the stack
  mov RAX, 0
  leave
  ret
...
memcheck_ok_15:
  ...
  ;; Store all the closed over variables {
  mov QWORD [R15+24], 24690 ; reserved space, will be filled in with: lam1_71
  ;; } Store all the closed over variables
  mov RAX, R15 ; Move the address of the tuple into RAX
  add RAX, 0x5 ; Tag it as a closure
  add R15, 32 ; Bump the heap register
  ;; } store lambda-15
  mov [RBP-72], RAX ; Store letrec-ed function lam0_66
  ;; Define lambda-17 {
...
lambda_17:
  ...
  sub RAX, 0x5 ; And detag it. Our compiler guarantees that it will be a lambda so no need to tag check
  mov R11, [RAX+24]
  mov [RBP-8], R11 ; Holds the closed over variable: a_53
  mov R11, [RAX+32]
  mov [RBP-16], R11 ; Holds the closed over variable: b_57
  mov R11, [RAX+40]
  mov [RBP-24], R11 ; Holds the closed over variable: c_61
  mov R11, [RAX+48]
  mov [RBP-32], R11 ; Holds the closed over variable: lam0_66
  mov R11, [RAX+56]
  ...
  add RSP, 16 ; Increment RSP after calling the closure
  mov RAX, [RBP+16]
  mov [RBP-48], RAX ; [RBP-48] holds the variable x_97
  mov RAX, [RBP+24]
  mov [RBP-56], RAX ; [RBP-56] holds the variable y_101
  mov RAX, [RBP+32]
  mov [RBP-64], RAX ; [RBP-64] holds the variable z_105
  mov RAX, [RBP-40]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
...
memcheck_ok_17:
  ...
  ;; Store all the closed over variables {
  mov R11, [RBP-48] ; Store a_53
  mov QWORD [R15+24], R11
  mov R11, [RBP-56] ; Store b_57
  mov QWORD [R15+32], R11
  mov R11, [RBP-64] ; Store c_61
  mov QWORD [R15+40], R11
  mov R11, [RBP-72] ; Store lam0_66
  ...
  ;; } store lambda-17
  mov [RBP-80], RAX ; Store letrec-ed function lam1_71
  mov RAX, [RBP-80]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
