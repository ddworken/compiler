...
memcheck_ok_89:
  ;; Store lambda-89 on the heap {
  mov QWORD [R15+0], 2 ; Store the arity
  mov QWORD [R15+8], lambda_89 ; Store the function pointer
...
lambda_end_77:
  ...
  mov R15, RAX ; assume gc success if returning here, so RAX holds the new heap_reg value
memcheck_ok_77:
  ;; Store lambda-77 on the heap {
  mov QWORD [R15+0], 2 ; Store the arity
  mov QWORD [R15+8], lambda_77 ; Store the function pointer
  mov QWORD [R15+16], 0 ; Store the number of closed variables
  ;; Store all the closed over variables {
  ;; } Store all the closed over variables
  mov RAX, R15 ; Move the address of the tuple into RAX
...
lambda_51:
  ...
  mov RAX, R11
  cmp RAX, R12
  jne near _cmp_fail_69
  mov RAX, 0xffffffffffffffff
  jmp near _cmp_done_69
_cmp_fail_69:
...
lambda_54:
  ...
  sub RAX, 0x5 ; And detag it. Our compiler guarantees that it will be a lambda so no need to tag check
  mov R11, [RAX+24]
  mov [RBP-8], R11 ; Holds the closed over variable: n_72
  mov R11, [RAX+32]
  mov [RBP-16], R11 ; Holds the closed over variable: acc_74
  mov R11, [RAX+40]
  mov [RBP-24], R11 ; Holds the closed over variable: f_53
  ;; } Copy the free variables off the heap onto the stack
  mov RAX, [RBP-8]
  mov R11, RAX
  ...
  mov [RBP-32], RAX ; [RBP-32] holds the variable binop_63
  mov RAX, [RBP+16]
  mov R11, RAX
  ;; Assert that R11 holds a number
  test QWORD R11, 0x1
  ...
  add RAX, R12
  jo near _global_overflow_abort
  mov [RBP-40], RAX ; [RBP-40] holds the variable binop_66
  mov RAX, [RBP-24]
...
memcheck_ok_54:
  ...
  ;; Store all the closed over variables {
  mov R11, [RBP+16] ; Store n_72
  mov QWORD [R15+24], R11
  mov R11, [RBP+24] ; Store acc_74
  mov QWORD [R15+32], R11
  mov R11, [RBP+32] ; Store f_53
...
memcheck_ok_51:
  ...
  call RAX ; func(20, 0)
  add RSP, 32 ; Increment RSP after calling the closure
  mov [RBP-56], RAX ; [RBP-56] holds the variable app_96
  mov RAX, [RBP-56]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  mov [RBP-64], RAX ; [RBP-64] holds the variable app_94
  mov RAX, [RBP-64]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func(4)
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-72], RAX ; [RBP-72] holds the variable app_92
  mov RAX, [RBP-72]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func(6)
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-80], RAX ; [RBP-80] holds the variable app_90
  mov RAX, [RBP-80]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func(8)
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-88], RAX ; [RBP-88] holds the variable app_88
  mov RAX, [RBP-88]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func(10)
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-96], RAX ; [RBP-96] holds the variable app_86
  mov RAX, [RBP-96]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func(12)
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-104], RAX ; [RBP-104] holds the variable app_84
  mov RAX, [RBP-104]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  mov RAX, [RAX+8]
  sub RSP, 16 ; Decrement RSP to prepare to call the closure
  mov R10, QWORD 14 ; arg_0
  mov QWORD [RSP+0], R10
  mov R11, [RBP-104] ; Pass the function itself as the final argument
  mov QWORD [RSP+8], R11
  call RAX ; func(14)
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-112], RAX ; [RBP-112] holds the variable app_82
  mov RAX, [RBP-112]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func(16)
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-120], RAX ; [RBP-120] holds the variable app_80
  mov RAX, [RBP-120]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  call RAX ; func(18)
  add RSP, 16 ; Increment RSP after calling the closure
  mov [RBP-128], RAX ; [RBP-128] holds the variable app_78
  mov RAX, [RBP-128]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  mov R10, QWORD 20 ; arg_0
  mov QWORD [RSP+0], R10
  mov R11, [RBP-128] ; Pass the function itself as the final argument
  mov QWORD [RSP+8], R11
  call RAX ; func(20)
  add RSP, 16 ; Increment RSP after calling the closure
  leave
