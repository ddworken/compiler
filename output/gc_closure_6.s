...
memcheck_ok_30:
  ...
  add RAX, 0x1
  add R15, 32
  mov [RBP-8], RAX ; [RBP-8] holds the variable wrapped_tuple_58
  ;; Define lambda-28 {
...
lambda_28:
  ...
  sub RAX, 0x5 ; And detag it. Our compiler guarantees that it will be a lambda so no need to tag check
  mov R11, [RAX+24]
  mov [RBP-8], R11 ; Holds the closed over variable: wrapped_tuple_58
  ;; } Copy the free variables off the heap onto the stack
  mov RAX, [RBP-8]
  leave
  ret
...
memcheck_ok_28:
  ...
  ;; Store all the closed over variables {
  mov R11, [RBP-8] ; Store wrapped_tuple_58
...
memcheck_ok_26:
  ...
  add R15, 32 ; Bump the heap register
  ;; } store lambda-26
  mov [RBP-48], RAX ; Store letrec-ed function make_closure_53
...
memcheck_ok_15:
  ...
  add RAX, 0x1
  add R15, 32
  mov [RBP-56], RAX ; [RBP-56] holds the variable tup_81
  mov RAX, [RBP-48]
  ...
  mov [RBP-64], RAX ; [RBP-64] holds the variable app_80
  mov RAX, [RBP-64]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
