...
lambda_76:
  ...
  mov RAX, R11
  cmp RAX, R12
  jne near _cmp_fail_93
  mov RAX, 0xffffffffffffffff
  jmp near _cmp_done_93
_cmp_fail_93:
...
lambda_34:
  ...
  mov QWORD [RBP-96], QWORD 0 ; Zero out the stack
  ;; Copy the free variables off the heap onto the stack where they can be used by the body of the lambda {
  mov RAX, [RBP+64] ; Move the last argument of the function which is the function itself into RAX so we can unpack closed over values
  sub RAX, 0x5 ; And detag it. Our compiler guarantees that it will be a lambda so no need to tag check
  mov R11, [RAX+24]
  ...
  mov RAX, [RAX+8]
  mov [RBP-16], RAX ; [RBP-16] holds the variable get_93
  mov RAX, [RBP+24]
  ;; Assert that RAX has the tag 1 (aka: 'tuple')
  mov R11, RAX
  shl R11, 61
  ...
  ;; Get the 0-th item from the tuple: 
  mov RAX, [RAX+8]
  mov [RBP-24], RAX ; [RBP-24] holds the variable get_95
  mov RAX, [RBP-16]
  mov R11, RAX
  ;; Assert that R11 holds a number
  test QWORD R11, 0x1
  ...
  add RAX, R12
  jo near _global_overflow_abort
  mov [RBP-32], RAX ; [RBP-32] holds the variable binop_92
  mov RAX, [RBP+32]
  ;; Assert that RAX has the tag 1 (aka: 'tuple')
  mov R11, RAX
  shl R11, 61
  ...
  ;; Get the 0-th item from the tuple: 
  mov RAX, [RAX+8]
  mov [RBP-40], RAX ; [RBP-40] holds the variable get_97
  mov RAX, [RBP-32]
  mov R11, RAX
  ;; Assert that R11 holds a number
  test QWORD R11, 0x1
  ...
  add RAX, R12
  jo near _global_overflow_abort
  mov [RBP-48], RAX ; [RBP-48] holds the variable binop_91
  mov RAX, [RBP+40]
  ;; Assert that RAX has the tag 1 (aka: 'tuple')
  mov R11, RAX
  shl R11, 61
  ...
  ;; Get the 0-th item from the tuple: 
  mov RAX, [RAX+8]
  mov [RBP-56], RAX ; [RBP-56] holds the variable get_99
  mov RAX, [RBP-48]
  mov R11, RAX
  ;; Assert that R11 holds a number
  test QWORD R11, 0x1
  ...
  add RAX, R12
  jo near _global_overflow_abort
  mov [RBP-64], RAX ; [RBP-64] holds the variable binop_90
  mov RAX, [RBP+48]
  ;; Assert that RAX has the tag 1 (aka: 'tuple')
  mov R11, RAX
  shl R11, 61
  ...
  ;; Get the 0-th item from the tuple: 
  mov RAX, [RAX+8]
  mov [RBP-72], RAX ; [RBP-72] holds the variable get_101
  mov RAX, [RBP-64]
  mov R11, RAX
  ;; Assert that R11 holds a number
  test QWORD R11, 0x1
  ...
  add RAX, R12
  jo near _global_overflow_abort
  mov [RBP-80], RAX ; [RBP-80] holds the variable binop_89
  mov RAX, [RBP+56]
  ;; Assert that RAX has the tag 1 (aka: 'tuple')
  mov R11, RAX
  shl R11, 61
  ...
  ;; Get the 0-th item from the tuple: 
  mov RAX, [RAX+8]
  mov [RBP-88], RAX ; [RBP-88] holds the variable get_103
  mov RAX, [RBP-80]
  mov R11, RAX
  ;; Assert that R11 holds a number
  test QWORD R11, 0x1
...
memcheck_ok_34:
  ...
  mov QWORD [R15+0], 12 ; Store the arity
  ...
  add R15, 32 ; Bump the heap register
  ;; } store lambda-34
  mov [RBP-56], RAX ; Store letrec-ed function f_78
...
memcheck_ok_32:
  ...
  add RAX, 0x1
  add R15, 16
  mov [RBP-64], RAX ; [RBP-64] holds the variable tup_118
  ;; Check if we have space on the heap to store our tuple of size 16: 
  mov RAX, [HEAP_END] ; Reserving 2 words
  sub RAX, 16
  cmp RAX, R15
...
memcheck_ok_30:
  ...
  add RAX, 0x1
  add R15, 16
  mov [RBP-72], RAX ; [RBP-72] holds the variable tup_120
  ;; Check if we have space on the heap to store our tuple of size 16: 
  mov RAX, [HEAP_END] ; Reserving 2 words
  sub RAX, 16
  cmp RAX, R15
...
memcheck_ok_28:
  ...
  add RAX, 0x1
  add R15, 16
  mov [RBP-80], RAX ; [RBP-80] holds the variable tup_122
  ;; Check if we have space on the heap to store our tuple of size 16: 
  mov RAX, [HEAP_END] ; Reserving 2 words
  sub RAX, 16
  cmp RAX, R15
...
memcheck_ok_26:
  ...
  add RAX, 0x1
  add R15, 16
  mov [RBP-88], RAX ; [RBP-88] holds the variable tup_124
  ;; Check if we have space on the heap to store our tuple of size 16: 
  mov RAX, [HEAP_END] ; Reserving 2 words
  sub RAX, 16
  cmp RAX, R15
...
memcheck_ok_24:
  ...
  add RAX, 0x1
  add R15, 16
  mov [RBP-96], RAX ; [RBP-96] holds the variable tup_126
  ;; Check if we have space on the heap to store our tuple of size 16: 
  mov RAX, [HEAP_END] ; Reserving 2 words
  sub RAX, 16
  cmp RAX, R15
...
memcheck_ok_22:
  ...
  add RAX, 0x1
  add R15, 16
  mov [RBP-104], RAX ; [RBP-104] holds the variable tup_128
  mov RAX, [RBP-56]
  ;; Assert that RAX has the tag 5 (aka: 'closure')
  mov R11, RAX
  shl R11, 61
  ...
  jne near _global_assertion_failed_arity_error
  mov RAX, [RAX+8]
  sub RSP, 64 ; Decrement RSP to prepare to call the closure
  mov R10, QWORD [RBP-64] ; arg_0
  mov QWORD [RSP+0], R10
  mov R10, QWORD [RBP-72] ; arg_1
  mov QWORD [RSP+8], R10
  mov R10, QWORD [RBP-80] ; arg_2
  mov QWORD [RSP+16], R10
  mov R10, QWORD [RBP-88] ; arg_3
  mov QWORD [RSP+24], R10
  mov R10, QWORD [RBP-96] ; arg_4
  mov QWORD [RSP+32], R10
  mov R10, QWORD [RBP-104] ; arg_5
  mov QWORD [RSP+40], R10
  mov QWORD [RSP+56], 0x0 ; Fill in the padded argument with a zero so all stack memory is padded with zeros
  mov R11, [RBP-56] ; Pass the function itself as the final argument
  mov QWORD [RSP+48], R11
  call RAX ; func([RBP-64], [RBP-72], [RBP-80], [RBP-88], [RBP-96], [RBP-104])
  add RSP, 64 ; Increment RSP after calling the closure
  leave
  ret
  ;; Error labels: 
